...
import yaml, os, json, subprocess, math
import healpy as hp
...
from .utils import sub_hpix, dist_ang, make_tile_cat
...
def store_gawa_confs(workdir, param_cfg, param_data):
    with open(
            os.path.join(workdir, 'config', 'gawa.cfg'), 'w'
    ) as outfile:
        json.dump(param_cfg, outfile)
    config = os.path.join(workdir, 'config', 'gawa.cfg')    
    with open(
...
def create_gawa_directories(workdir):
    create_directory(workdir)
    create_directory(os.path.join(workdir, 'tiles'))
    create_directory(os.path.join(workdir, 'sky_partition_gawa'))
...
def update_config(param_cfg, param_data):
    ...
    param_data_org = copy.deepcopy(param_data)
    ...
    ref_bfilter = param_cfg['ref_bfilter']
    ref_rfilter = param_cfg['ref_rfilter']
    ref_color = param_cfg['ref_color']

    # update mag with selected filter in config 
    param_data['starcat'][survey]['keys']['key_mag'] = \
          param_data['starcat'][survey]['keys']['key_mag'][ref_filter]
    # update final cluster cat output
    param_cfg['clcat']['gawa']['cat'] = os.path.join(
        workdir, 'clusters.fits'
    )
    # add wazp case to clcat in data.cfg for Pmem execution
    param_data['clcat'] = {'gawa': param_cfg['clcat']['gawa']}

    param_data['isochrone_masks'][survey]['model_file'] = \
        param_data['isochrone_masks'][survey]['model_file'][ref_color]

    param_data['isochrone_masks'][survey]["magerr_blue_file"] = \
        param_data['isochrone_masks'][survey]['magerr_file'][ref_bfilter]
    param_data['isochrone_masks'][survey]["magerr_red_file"] = \
        param_data['isochrone_masks'][survey]['magerr_file'][ref_rfilter]
    param_data['isochrone_masks'][survey]['mask_color_min'] = \
        param_data['isochrone_masks'][survey]['mask_color_min'][ref_color]
    param_data['isochrone_masks'][survey]['mask_color_max'] = \
        param_data['isochrone_masks'][survey]['mask_color_max'][ref_color]
    param_data['isochrone_masks'][survey]['mask_mag_min'] = \
        param_data['isochrone_masks'][survey]['mask_mag_min'][ref_filter]
    param_data['isochrone_masks'][survey]['mask_mag_max'] = \
        param_data['isochrone_masks'][survey]['mask_mag_max'][ref_filter]
    param_data['isochrone_masks'][survey]['mask_resolution'] = \
        param_cfg['gawa_cfg']['mask_resolution']
    param_data['isochrone_masks'][survey]['label_mag'] = ref_filter.split('_')[1]
    param_data['isochrone_masks'][survey]['label_col'] = ref_color

    #param_cfg['admin']['tiling']['nest'] = input_data_structure[survey]['nest']
    param_data['starcat'][survey]['mosaic']['Nside'] = \
        input_data_structure[survey]['Nside']
    param_data['starcat'][survey]['mosaic']['nest'] = \
    ...
    # update filters 
    param_data['starcat'][survey]['keys']['key_mag_blue'] = \
        param_data_org['starcat'][survey]['keys']['key_mag'][ref_bfilter]
    param_data['starcat'][survey]['keys']['key_mag_red'] = \
        param_data_org['starcat'][survey]['keys']['key_mag'][ref_rfilter]
...
def create_tile_directories(root, path):  # only called from pmem_main
...
def merged_cmd_polygons(dslices, ids, isochrone_masks, workdir):

    
    create_directory(os.path.join(workdir, "isochrone_masks_init"))

    ymin, ymax = 15., 24.
    xmin, xmax = -0.35, 1.5
    
    cmdfile = isochrone_masks['model_file']
    lab_col = isochrone_masks["label_col"]
    lab_mag = isochrone_masks["label_mag"]
    
    gr, g0 = np.loadtxt(cmdfile, usecols=(0, 1), unpack=True)
    
    dslice = dslices['dist_kpc']*1000.
    distmin = dslices['distmin_kpc']*1000.
    distmax = dslices['distmax_kpc']*1000.
    gcen = g0 + 5.*np.log10(dslice/10.)
    
    i=0
    for dd in np.arange(distmin, distmax, 50):
        g = g0 + 5.*np.log10(dd/10.)
        vertices = np.dstack((gr, g)).reshape(len(g),2)
        polygon = Polygon(vertices)
        if i == 0:
            newp = Polygon(vertices)
        else:
            newp = newp.union(Polygon(vertices))
        i+=1
    xx, yy = newp.exterior.coords.xy
        
    plt.clf()
    plt.plot(
        gr, gcen,
        color='red',
        label="mask at D (kpc) = "+str(np.round(dslice/1000, 2))) 
    plt.plot(
        xx, yy,
        color='green', 
        label="mask in D range = "+str(np.round(distmin/1000, 2))+" - "+\
        str(np.round(distmax/1000, 2))
    )
    plt.xlabel(lab_col, fontsize=15)
    plt.ylabel(lab_mag, fontsize=15)
    ...
    plt.savefig(
        os.path.join(
            workdir,
            "isochrone_masks_init",
            'cmd_sl'+str(ids)+'_D'+str(np.round(dslice/1000, 2))+'.png'
        )
    )

    slice_model_file = os.path.join(
        workdir,
        "isochrone_masks_init",
        "cmd_slice_"+str(ids)+".txt"
    )
    f = open(slice_model_file, "w")
    for j in range(len(xx)):
        f.write(
            str(np.round(xx[j],2))+" "+str(np.round(yy[j],2))+"\n"
        )        
    return slice_model_file
                     

def compute_dslices(isochrone_masks, dslices_specs, maglim_det, workdir):
    """_summary_
    Args:
    ...
    grmax_HB = 0.1 # color cut to select
    grmin_HB = -0.1 # color cut to select
    gmin = dslices_specs['HB_mag_min']
    gmax = dslices_specs['HB_dmag_max']+maglim_det
    
    cmdfile = isochrone_masks['model_file']
    lab_mag = isochrone_masks["label_mag"]
    gr, g0 = np.loadtxt(cmdfile, usecols=(0, 1), unpack=True)
    
    # define mean mag of the HB
    color_cond = (gr<=grmax_HB) & (gr>=grmin_HB)
    g0_HB = np.median(g0[color_cond])
    dginit = (np.amax(g0[color_cond]) - np.amin(g0[color_cond]))/2.
    # read error files 
    ...
    gg = np.arange(gmin, gmax, 0.1)
    k=0
    gout = np.zeros(100)
    goutmin = np.zeros(100)
    goutmax = np.zeros(100)
    dd = np.zeros(100)
    dg = np.zeros(len(gg))
    ddmin = np.zeros(100)
    ddmax = np.zeros(100)
    ...
    for i in range(len(gg)):
        dg[i] = dginit + np.interp (gg[i], gm, gm_err)
        if i == 0:
            gout[k] = gg[i]
            goutmin[k] = gg[i]-dg[i]
            goutmax[k] = gg[i]+dg[i]
            
            dd[k] = 10.*np.power(10, (gout[i]-g0_HB)/5.)
            ddmin[k] = 10.*np.power(10, (goutmin[i]-g0_HB)/5.)
            ddmax[k] = 10.*np.power(10, (goutmax[i]-g0_HB)/5.)
            k+=1
        else:
            gtest = gg[i]
            goutmintest = gg[i]-dg[i]
            goutmaxtest = gg[i]+dg[i]
            ddmintest = 10.*np.power(10, (goutmintest-g0_HB)/5.)
            ddmaxtest = 10.*np.power(10, (goutmaxtest-g0_HB)/5.)            
            if (ddmintest>dd[k-1]) & (ddmintest<ddmax[k-1]):
                gout[k] = gg[i]
                goutmin[k] = gg[i]-dg[i]
                goutmax[k] = gg[i]+dg[i]
                dd[k] = 10.*np.power(10, (gout[k]-g0_HB)/5.)
                ddmin[k] = 10.*np.power(10, (goutmin[k]-g0_HB)/5.)
                ddmax[k] = 10.*np.power(10, (goutmax[k]-g0_HB)/5.)
                k+=1

    plt.clf()
    c = (gout>0)
    plt.scatter(gout[c], dd[c]/1000, c='red', label='slice center') 
    plt.errorbar(
        gout[c], dd[c]/1000,
        (ddmax[c]/1000- ddmin[c]/1000)/2. , c='red'
    ) 
    plt.plot(
        gout[c], ddmin[c]/1000,
        color='red', linestyle='dashed', label='slice width'
    ) 
    plt.plot(
        gout[c], ddmax[c]/1000, color='red', linestyle='dashed'
    ) 
    plt.xlabel(lab_mag+'$_{HB}$', fontsize=20)
    plt.ylabel('Distance (kpc)', fontsize=20)
    plt.tight_layout()
    plt.legend()
    plt.savefig(os.path.join(workdir, 'dslices.png'))

    dslices = np.array(dd[c])
    data = np.zeros( (len(dd[c])), 
                     dtype = {
                         'names':(
                             'id', 'dist_kpc',
                             'distmin_kpc', 'distmax_kpc'
                         ), 
                         'formats':(
                             'i8', 'f8',
                             'f8', 'f8'
    ...
    data['id'] = np.arange(len(dd[c]))
    data['dist_kpc'] = dd[c]/1000.
    data['distmin_kpc'] = ddmin[c]/1000.
    data['distmax_kpc'] = ddmax[c]/1000.
    t = Table (data)
    t.write(
        os.path.join(workdir, dslices_specs['dslices_filename'] ), 
        overwrite=True
    )                
    return
...
def plot_cmd(xall,yall, xar_in, yar_in, isochrone_masks, file_png):
    ...
    plt.xlabel(lab_col, fontsize=20)
    plt.ylabel(lab_mag, fontsize=20)
    plt.axis((xmin, xmax, ymax, ymin))
    plt.tight_layout()
...
def cmd_mask(dslice, slice_id,
             isochrone_masks, model_file, nx, nsig, out_paths):
    """_summary_
    ...
    gr, g = np.loadtxt(model_file, usecols=(0, 1), unpack=True)
    vertices = np.dstack((gr, g)).reshape(len(g),2)
    p = pth.Path(vertices)
    ...
    for i in range (0, len(xar_in)):
        ...
        cond_err = ((ixar<ixar_in[i] + nxerr) & (ixar>ixar_in[i] - nxerr) & 
                    (iyar<iyar_in[i] + nyerr) & (iyar>iyar_in[i] - nyerr))
        if i == 0:
    ...
    plot_filename = os.path.join(
        ...
        'cmd_sl'+str(slice_id)+'_D'+str(np.round(dslice/1000, 2))+'_err.png'
    )
...
def cmd_mask2(dslice, isochrone_masks, nx, nsig, out_paths):
    """_summary_
    ...
    Returns:
        _type_: _description_
    """
    # consider distance variation in a slice 
    xmin, xmax = isochrone_masks["mask_color_min"],\
                 isochrone_masks["mask_color_max"]
    ymin, ymax = isochrone_masks["mask_mag_min"],\
                 isochrone_masks["mask_mag_max"]
...
def plot_all_cmds(dslices_filename, isochrone_masks, nx, out_paths, outfile):

    print ('Plot all CMD masks')

    dslices = read_FitsCat( 
        os.path.join(out_paths['workdir'], dslices_filename)
    )['dist_kpc']
    ...
    plt.clf()
    colors = plt.cm.jet(np.linspace(0,1,len(dslices)))
    for i in range(0, len(dslices)):
        mask = np.load(
            os.path.join(
                out_paths['workdir'], out_paths['masks'],
                'cmd_mask_D'+str(np.round(dslices[i], 2))+'.npy'
            )
        )
        ixar, iyar =  np.argwhere(mask==1).T[0], np.argwhere(mask==1).T[1]
        xalln, yalln = pix2xy(ixar, iyar, xmin, xmax, ymin, ymax, nx, ny)
        if (i==0) or (i==len(dslices)-1):
            plt.scatter(
                xalln, yalln,
                s=5, color=colors[i], alpha=0.1,
                label='D (kpc) = '+str(np.round(dslices[i], 2))
            )
        else:
            plt.scatter(xalln, yalln,
                        s=5, color=colors[i], alpha=0.1)            
    plt.xlabel(lab_col, fontsize=20)
    plt.ylabel(lab_mag, fontsize=20)
    plt.axis((xmin+0.02, xmax, ymax, ymin))
    plt.tight_layout()
    plt.legend()
    plt.savefig(outfile)
    return
...
def compute_cmd_masks(isochrone_masks, out_paths, gawa_cfg):
    ...
    for i in range(0, len(dslices)):
        print ('.....Mask distance (kpc) = ', 
               np.round(dslices['dist_kpc'][i], 2))
        if not os.path.isfile(
                os.path.join(
                    out_paths['workdir'], out_paths['masks'],
                    'cmd_mask_D'+\
                    str(np.round(dslices['dist_kpc'][i], 2))+'.npy')):

            slice_model_file = merged_cmd_polygons(
                dslices[i], i, 
                isochrone_masks, out_paths['workdir']
            )
            mask = cmd_mask(
                dslices['dist_kpc'][i]*1000., i, isochrone_masks,
                slice_model_file,
                gawa_cfg['mask_resolution'],
...
def pixelized_radec(ra_map, dec_map, weight_map, w, nxy):
    ...
    xycat, xedges, yedges = np.histogram2d(
        xmap, ymap, bins=nxy, range=((0,nxy),(0,nxy)), 
        weights=weight_map
    )
    return xycat
...
def pixelized_colmag(color, mag, weight, isochrone_masks, out):
    ...
    ny = int((ymax-ymin)/step)
    xmap, ymap = xy2pix(
        color, mag, xmin, xmax, ymin, ymax, nx, ny
    )
    colmag_pix, cedges, medges = np.histogram2d(
        xmap, ymap, bins=(nx, ny), range=((0,nx),(0,ny)), 
        weights=weight
    )
...
def plot_pixelized_colmag(color_aper, gmag_aper, 
                          weight_aper, color_slaper, 
                          gmag_slaper, weight_slaper, 
                          color, mag, weight, 
                          isochrone_masks, out):
    """_summary_
...
def create_wcs(gawa_cfg, tile_specs):
    ...
    pix_deg = 1./(60.*float(gawa_cfg['map_resolution']))
...
def compute_filled_catimage(ra_map, dec_map, weight_map, 
                            gawa_cfg, tile, data_fp, 
    ...
    nlist = hp.pixelfunc.get_all_neighbours(
        footprint['Nside'], data_fp[footprint['key_pixel']], 
        None, footprint['nest']
    )
    ...
    shpix_filled = np.unique(
        hp.ang2pix(
            footprint['Nside']*2, ra_map, dec_map, footprint['nest'], lonlat=True
        )
    )
    hpix_filled = np.unique(
        hp.ang2pix(
            footprint['Nside'], ra_map, dec_map, footprint['nest'], lonlat=True
            )
    )
    ...
    hpx_ran1 = hp.ang2pix(
        footprint['Nside'], ra_ran, dec_ran, footprint['nest'], lonlat=True
    )
    ra_ranf1 =   ra_ran\
    ...
    hpx_ran2 = hp.ang2pix(
        footprint['Nside']*2, ra_ran, dec_ran, footprint['nest'], lonlat=True
    )
...
def run_mr_filter(filled_catimage, wmap, gawa_cfg):
    ...
    if smin == 1:
        subprocess.run((
            'mr_filter'+\
            ' -m 10 -i 3 -s 10.,3.,3. -n '+str(smax+1)+' -f 3 -K -C 2 -p -e0 -A '+\
            filled_catimage+' '+wmap
        ), check=True, shell=True, stdout=subprocess.PIPE).stdout
    if smin == 2:
        subprocess.run((
            'mr_filter'+\
            ' -m 10 -i 3 -s 10.,10.,3.,3. -n '+str(smax+1)+' -f 3 -K -C 2 -p -e0 -A '+\
            filled_catimage+' '+wmap
        ), check=True, shell=True, stdout=subprocess.PIPE).stdout
    if smin == 3:
        subprocess.run((
            'mr_filter'+\
            ' -m 10 -i 3 -s 10.,10.,10.,3.,3. -n '+str(smax+1)+' -f 3 -K -C 2 -p -e0 -A '+\
            filled_catimage+' '+wmap
        ), check=True, shell=True, stdout=subprocess.PIPE).stdout
...
def wmap2peaks(wmap, wazp_specs, tile_specs):
    ...
    local_maxi = peak_local_max(wmap_data, min_distance=24)  # this has to be reviewed 
...
def filter_peaks(admin, tile, map_resolution, ra0, dec0, ip0, jp0):
    """_summary_
...
def init_peaks_table(ra_peaks, dec_peaks, iobj, jobj, coverfrac, coverfrac_bkg, 
                     wradius, dslice, isochrone_masks, gawa_cfg, clkeys):
    ...
    data_peaks[clkeys['key_radius']] = wradius
    data_peaks['dist_init_kpc'] = (dslice/1000.)*np.ones(len(ra_peaks))
    data_peaks['dist_init_kpc'] = (dslice/1000.)*np.ones(len(ra_peaks))
...
def select_stars_in_disc(racen, deccen, data_stars, starcat, aper):
    ...
    Returns:
        _type_: _description_
    """
    ras, decs = np.float64(data_stars[starcat['keys']['key_ra']]),\
                np.float64(data_stars[starcat['keys']['key_dec']])
...
def compute_flux_aper_vec(rap, decp, aper, ras, decs, weights):
    ...
    Nside = 16384
    nest = False
    hpix = hp.ang2pix(Nside, ras, decs, nest, lonlat=True)
    Naper = np.zeros(len(rap))
    for i in range(0, len(rap)):
        Naper[i] = compute_flux_aper(
            rap[i], decp[i], hpix, weights, aper, Nside, nest
        )
    return Naper
...
def gmag_HB(isochrone_masks, dslice):
    ...
    model_file = isochrone_masks["model_file"]
    gr, g0 = np.loadtxt(model_file, usecols=(0, 1), unpack=True)
    g0_HB_min = np.amin(g0[(gr<0.05) & (gr>-0.5)])
    g0_HB_max = np.amax(g0[(gr<0.05) & (gr>-0.5)])
...
def Dist_err(isochrone_masks, dslice):
    ...
    g = g0 + 5.*np.log10(dslice/10.)
    gmin = np.amin(g[(gr<0.05) & (gr>-0.5)])
    gmax = np.amax(g[(gr<0.05) & (gr>-0.5)])
    gmin = gmin - np.interp (gmin, gm, gm_err)
    gmax = gmax + np.interp (gmax, gm, gm_err)    
    g_HB = (gmin + gmax)/2. 
    dg = gmax - g_HB
    Dmin = (dslice/10.)/10**(dg/10.) 
    Dmax = (dslice/10.)*10**(dg/10.)
    delta_D = Dmax - Dmin
    return delta_D/100., Dmin/100., Dmax/100.


def compute_dslices_old(isochrone_masks, dslices_specs, workdir):
    """_summary_
    Args:
        isochrone_masks (bool): _description_
        dslices_specs (_type_): _description_
    ...
    gmin, gmax = np.amin(g[(gr<0.05) & (gr>-0.5)]),\
                 np.amax(g[(gr<0.05) & (gr>-0.5)])
    ...
    for j in range(0,nstep):
        dist+=dstep
        g = g0 + 5.*np.log10(dist/10.)
        gmin1 = np.amin(g[(gr<0.05) & (gr>-0.5)])
        gmax1 = np.amax(g[(gr<0.05) & (gr>-0.5)])
...
def add_peaks_attributes(data_peaks, data_stars, starcat, bkg_arcmin2, 
                         dslice, isochrone_masks, tile, gawa_cfg, clkeys):
    """_summary_
    ...
    Nraw_snr = compute_flux_aper_vec(
        data_peaks['ra'], data_peaks['dec'],
        gawa_cfg['radius_aper_arcmin']/60.,
        ras, decs, np.ones(len(ras))
    )
    ...
    Nbkg = int(np.pi*bkg_arcmin2*gawa_cfg['radius_aper_arcmin']**2) +1.
    Nsnr = Nraw_snr - Nbkg
    if Nbkg < 9.:
        sig_N = 2.*Nbkg**0.5 - Nbkg/3.
    else:
        sig_N = Nbkg**0.5
...
def append_peaks_infos_to_cylinders(data_cylinders_init, peaks_list, dslices,
    ...
    dist_init = dslices[data_cylinders_init['cyl_isl_mode']]/1000.
    nsl = len(dslices)
...
def make_cylinders(peaks_list, dslices, gawa_cfg, clkeys):
    """_summary_
    ...
    rad_deg = gawa_cfg['merge_specs']['match_radius']/60.
    flag_min = 0
    npeaks_all = 0
    nsl = len(peaks_list)

    key_snr =  clkeys['key_snr']
...
def gawa_tile_slice(admin, tile, dslice, isochrone_masks, data_star, starcat, 
                    data_fp, footprint, gawa_cfg, clkeys, out_paths, verbose):
    """_summary_
    ...
    pixelized_colmag(
        gmag-rmag, gmag, np.ones(len(gmag)),
        isochrone_masks, cmd_histogram
    )
    xycat = compute_catimage(
        ras, decs, np.ones(len(ras)), gawa_cfg, tile
    ) 
    bkg_arcmin2 = float(len(ras))/(tile['framed_eff_area_deg2']*3600.)
    xycat_fi = compute_filled_catimage(
        ras, decs, np.ones(len(ras)), gawa_cfg, tile, 
        data_fp, footprint, bkg_arcmin2
    )

    # build density map /  extract peaks /compute attributes and filter 
    xycat_fi_fitsname = os.path.join(
        out_paths['workdir_loc'], out_paths['gawa']['files'],\
    ...
    rap, decp, ip, jp = filter_peaks(   # keep inner tile peaks
        admin, tile, gawa_cfg['map_resolution'], 
        rap0, decp0, ip0, jp0
    )
    wradius_arcmin = wave_radius(
        wmap_data, ip.astype(int), jp.astype(int), gawa_cfg
    )
    ...
    coverfrac_bkg = coverfrac_disc(
        rap, decp,
        data_fp, footprint, gawa_cfg['radius_bkg_max_arcmin']/60.
    )
    pcond = ((coverfrac>gawa_cfg['coverfrac_min']))
    data_peaks = init_peaks_table(
        rap[pcond], decp[pcond], ip[pcond], jp[pcond], coverfrac[pcond],\
        coverfrac_bkg[pcond], wradius_arcmin[pcond], 
        dslice, isochrone_masks, gawa_cfg, clkeys
    )
    
    # compute fluxes in given aper 
    add_peaks_attributes(
        data_peaks, data_star_slice, starcat, bkg_arcmin2, dslice, 
        isochrone_masks, tile, gawa_cfg, clkeys
    )
    ...
    if verbose >=2:
        for i in range(0, len(data_peaks[condf])):
            data_star_slice_aper = select_stars_in_disc(
                data_peaks[condf][i]['ra'], data_peaks[condf][i]['dec'],\
                data_star_slice, starcat,\
                gawa_cfg['radius_aper_arcmin']/60.
            )
            ...
            cmd_histogram = os.path.join(
                out_paths['workdir_loc'], out_paths['gawa']['plots'],\
                'cmd_hist_D'+str(dslice/1000)+'_radec'+\
                str(np.round(data_peaks[condf][i]['ra'],3))+\
                '_'+str(np.round(data_peaks[condf][i]['dec'],3))+'.png'
            )
            plot_pixelized_colmag(
                color_aper, gmag_aper, np.ones(len(gmag_aper)),
                color_slaper, gmag_slaper, np.ones(len(gmag_slaper)),
                gmag-rmag, gmag, np.ones(len(gmag)),
                isochrone_masks, cmd_histogram
            )
    ...
    if verbose >=1:
        ...
        print ('         distance / bkg_density/arcmin2 = ', 
    ...
    if verbose >=2:
        ...
        weights = gmag_weight(
            data_star_slice[starcat['keys']['key_mag_blue']], dslice,
            isochrone_masks, gawa_cfg
        )   
...
def gawa_tile(admin, tile_specs, isochrone_masks, starcat, footprint,
              maglim, gawa_cfg, clkeys, out_paths, verbose):
    """_summary_
    ...
    data_star_tile, data_fp_tile = make_tile_cat(
        tile_specs,
        starcat, footprint,
        maglim, gawa_cfg
    )
    ...
    if not os.path.isfile(os.path.join(
        ...
        for isl in range (0, len(dslices)):
            if not os.path.isfile(os.path.join(
                ...
                data_peaks = gawa_tile_slice(
                    admin,
                    tile_specs, dslices[isl], 
                    isochrone_masks, 
                    data_star_tile, starcat,
                    data_fp_tile, footprint, 
                    gawa_cfg, clkeys, out_paths, verbose
                )
        ...
        if npeaks_tot>0:
            ...
            data_cylinders = make_cylinders(
                peaks_list, dslices, gawa_cfg, clkeys
            )
            if verbose >=1:
            ...
            if data_cylinders is not None:    
                ...
                if verbose >=1:
                    ...
                    t.write(os.path.join(
                        out_paths['workdir_loc'], out_paths['gawa']['results'], 
                        "clusters0.fits"
                    ), overwrite=True)
    ...
    if data_clusters0 is not None:
        data_clusters = cl_duplicates_filtering(
            data_clusters0, gawa_cfg, clkeys, 'tile'
        )
    ...
    if data_clusters is not None:
        ...
        t.write(
            os.path.join(
                tile_dir, out_paths['gawa']['results'],
                "clusters.fits"
            ), overwrite = True
        )
...
def cylinders2clusters(data_cylinders, peaks_list, tile,
                       dslices, out_paths, clkeys):
    """_summary_
    ...
    for isl in range(0, len(dslices)):
        if len(peaks_list[isl]) > 0:
            ...
            data_peaks_ext['icyl'] = \
                data_cylinders['id_cyl'][data_cylinders['cyl_isl_mode']==isl]
...
def cl_duplicates_filtering(data_clusters_in, gawa_cfg, clkeys, mode):
    """_summary_
...
def tiles_with_clusters(out_paths, all_tiles):
    ...
    for it in range(0, len(all_tiles)):
        ...
        if os.path.isfile(os.path.join(
                tile_dir,
                out_paths['gawa']['results'], "tile_info.fits"
        )):
